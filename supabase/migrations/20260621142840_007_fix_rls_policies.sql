...
-- Jobs: Users can manage their own job postings
...
CREATE POLICY "delete_chats" ON chats FOR DELETE
  TO authenticated USING (auth.uid()::text = poster_user_id);

-- Messages: Users can only access messages in chats they participate in
CREATE POLICY "select_messages" ON messages FOR SELECT
  TO authenticated USING (
    auth.uid()::text IN (
      SELECT poster_user_id FROM chats WHERE id = chat_id
      UNION
      SELECT applicant_user_id FROM chats WHERE id = chat_id
    )
  );
...
CREATE POLICY "update_messages" ON messages FOR UPDATE
  TO authenticated USING (auth.uid()::text = sender_user_id)
  WITH CHECK (auth.uid()::text = sender_user_id);
CREATE POLICY "delete_messages" ON messages FOR DELETE
  TO authenticated USING (auth.uid()::text = sender_user_id);

-- Wallet transactions: Users can only access their own transactions
